...
if [ -n "$IS_CONNECTED" ]; then
    ...
    if [ "$RESPONSE" == "Disconnect" ]; then
        nmcli connection down "$CHOSEN_NETWORK" && notify-send -i network-wireless "󰖪 WiFi" "Disconnected from $CHOSEN_NETWORK"
    fi
else
    ...
    if [ -n "$HAS_PROFILE" ]; then
        ...
        notify-send -i network-wireless "󰖩 WiFi" "Connected to $CHOSEN_NETWORK"
    else
        PASSWORD=$(rofi -dmenu -password -p " Password for $CHOSEN_NETWORK" -theme ~/.config/rofi/config.rasi)
        if [ -n "$PASSWORD" ]; then
            nmcli device wifi connect "$CHOSEN_NETWORK" password "$PASSWORD" && notify-send -i network-wireless "󰖩 WiFi" "Connected to $CHOSEN_NETWORK"
        fi
    fi
fi
